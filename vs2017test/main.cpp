...
void drawTree(int xc, int yc, int zc, int height, double r, double g, double b)
{
	...
	for (alpha = 0; alpha <= 2 * PI; alpha += teta)
	{
		...
		glBegin(GL_POLYGON);
		glVertex3d(xc + sin(alpha), (double)yc + height, zc + cos(alpha)); // vertex 1
		glVertex3d(xc + sin(alpha + teta), (double)yc + height, zc + cos(alpha + teta)); // vertex 2
		glVertex3d(xc + sin(alpha + teta), (double)yc, zc + cos(alpha + teta)); // vertex 3
		glVertex3d(xc + sin(alpha), (double)yc, zc + cos(alpha)); // vertex 4
		glEnd();
	}
	...
	for (line = 1; line <= 4; line++)
	{
		for (alpha = 0; alpha <= 2 * PI; alpha += teta)
		{
			...
			glBegin(GL_POLYGON);
			glVertex3d((double)xc - line + sin(alpha), (double)lastYC + 1, zc + cos(alpha)); // vertex 1
			glVertex3d((double)xc + line + sin(alpha + teta), lastYC, zc + cos(alpha + teta)); // vertex 2
			glVertex3d((double)xc + line + sin(alpha + teta), lastYC + 1.0, zc + cos(alpha + teta)); // vertex 3
			glVertex3d((double)xc - line + sin(alpha), lastYC, zc + cos(alpha)); // vertex 4
			glEnd();
		}
		lastYC++;
	}
	for (line = 3; line >= 1; line--)
	{
		for (alpha = 0; alpha <= 2 * PI; alpha += teta)
		{
			...
			glBegin(GL_POLYGON);
			glVertex3d((double)xc - line + sin(alpha), lastYC + 1.0, zc + cos(alpha)); // vertex 1
			glVertex3d((double)xc + line + sin(alpha + teta), lastYC, zc + cos(alpha + teta)); // vertex 2
			glVertex3d((double)xc + line + sin(alpha + teta), lastYC + 1.0, zc + cos(alpha + teta)); // vertex 3
			glVertex3d((double)xc - line + sin(alpha), lastYC, zc + cos(alpha)); // vertex 4
			glEnd();
...
void DrawTexSphere(int n, int sl, int tnum, int num_rep, int vert_rep)
{
	...
	for (beta = -PI / 2, counter = 0; beta <= PI / 2; beta += gamma, counter++)
	{
		...
		DrawTexCylinder2(n, tnum, num_rep, cos(beta + gamma) / 4, cos(beta) / 4, (counter + 1.0) * part, counter * part);
		glPopMatrix();
	}
}
